...
# 安全字型設定：有 Noto 就用，沒有就退回預設，不要炸掉
try:
    font_path = Path(__file__).parent / "fonts" / "NotoSansCJK-Regular.otf"
    if font_path.exists():
        FONT_PROP = fm.FontProperties(fname=str(font_path))
    else:
        FONT_PROP = fm.FontProperties()  # 用系統預設字型
except Exception:
...
ax.set_title("資訊越準 → 不完美資訊價值（EVII）成長", fontproperties=FONT_PROP)
